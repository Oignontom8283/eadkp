...
const POSSIBLE_C_DIRS: [&str; 2] = ["src/libs", "src/lib"];
...
fn convert_image(file_path: &std::path::Path) {
    ...
    converted_pixels.extend(u32::from_le_bytes(*b"EIF1").to_le_bytes()); // Magic number (4 bytes)
...
fn main() {
    ...
    let asset_dir = std::path::Path::new(ASSETS_DIR);
...
\
  ...
  KeySDLKeyPair(Key::Right,     SDL_SCANCODE_RIGHT),\
        ...
        // Lire le fichier de configuration du clavier du simulateur
